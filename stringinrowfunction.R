...
a <- c("DANGEROUS_PIRATE", w = "Greedy_Pirate")
...
string.in.row(dc, df, b, a) -> trial

multiple.strings <- function(search_vecs, col_names){
  mapply(string.in.row, search_vecs, col_names, MoreArgs = c(desiredcols=dc, df =df), SIMPLIFY = TRUE,
         USE.NAMES = TRUE)
  }



multiple.strings(b,a) -> trial
